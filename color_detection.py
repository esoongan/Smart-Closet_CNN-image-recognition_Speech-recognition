...
from rgb_converter import rgb_converter
...
converter = rgb_converter('/Users/hayeong/Smart-Closet/color-rgb.txt')
# 이미지 로드
image = cv2.imread(
    "/Users/hayeong/Smart-Closet_local/database/newimg/Tonal_Check-Patterned_Blouse/img_00000030.jpg")  # test image

# 배경색 제거를 위한 이미지 자르기
h, w, _ = image.shape
image = image.copy()
image = image[int(h*0.1) : int(h*0.9), int(w*0.1) : int(w*0.9)] # [시작 height : 끝 height, 시작 width : 끝 width]
...
# 4. 각각의 %가 비슷비슷하게 나왔으면 각 색상을 모두 출력하기 ex)검정 30% 흰색 30% => 검정색, 흰색 모두 출력
# 각 클러스터의 rgb값에 해당 클러스터의 비율 곱함 이건 1번 방법
def get_average1():
    rgb = np.zeros(3)
    for i in range(k):
...
# 색상의 평균값 출력
# new_rgb = get_average2()
# print('clusters =',centers, 'W =', W)
# print('*** OUTPUT: Color Average [R,G,B]=', new_rgb, '***')
# rgb를 색상이름으로 매핑하는 함수 호출
# color_name = converter.rgb_to_name(new_rgb)
# print('*** get_average2() OUTPUT ***')
# print('Color Name=', color_name)


# count, new_rgbs = get_average4()
# print('*** get_average4() OUTPUT ***')
# for i in range(count):
#     color_name = converter.rgb_to_name(new_rgbs[i])
#     print('Color Name[',i,']=', color_name)

print('*** get_newRGB() OUTPUT ***')

# 0.6(=baseline) 이상 차지하는 색상이 있으면 그 색상 하나만 리턴
rgb_tuple_list = get_newRGB(0.6)
